...
instancesAreEqual' solver sp t1 t2 = do
  (res, subst) <- equalInstances sp t1 t2
  if res then do
      preds <- get >>= (substitute subst . predicateStack)
      let toSolve = Conj preds
...
kindCheckConstraint sp inst = do
  ...
  subs <- forM expectedKindPairs (\(ity, iKind) -> do
    (inferred, _) <- peekChecker (inferKindOfType sp ity)
    case inferred of
      Left errs -> throw (head (toList errs))
      Right tyKind -> do
        eqres <- equalKinds sp iKind tyKind
        case eqres of
          (True, _, subst) -> pure subst
          (False, _, _) -> throw KindMismatch{ errLoc = sp, tyActualK = Just ity, kExpected = iKind, kActual = tyKind })
  ...
  case instFromTy ty of
    ...
    Nothing -> do
      ...
      case kind of
        ...
        _ -> throw KindMismatch{ errLoc = s, tyActualK = Just ty, kExpected = KConstraint Predicate, kActual = kind }
