...
mapExprAnnotation f (Hole s ann) =
    Hole s (f ann)
...
rewriteExpr (Hole s _) = pure (Hole s ())
...
instantiate sig@(Forall _ binds _ ty) ity = do
  ...
  res <- runCheckerInRewriter $ do
           (_, _, subst) <- C.withBindings binds BoundQ $ equalTypes nullSpanNoFile tyNoI ity
           Sub.substitute subst sig
